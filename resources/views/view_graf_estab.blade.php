  		...
  		<div class="col-xs-2">
  			<div style="width: 100%;">
	  			<div class="form-group">
                    ...
                    <br>
                    <br>
                    {{ Form::label('estab','Establecimiento',null,['class'=>'form-control input-lg']) }}  
                    {{ Form::select('estab',['data-placeholder'=>'Selecciona...'],null,['class'=>'form-control input-md']) }}  	
...
<script>
	...
	$.get("dat_eess/"+event.target.value+"", function(response, state){
        ...
        {
            $("#estab").append("<option value='"+response[i].id_eess+"'>"+response[i].eess+"</option>");
        }
...
$("#estab").change(function(event){
    $.get("grf_eess/"+event.target.value+"", function(response, state){
        graf_est(response);
    });
});
...
function graf_est(datos)
{
    ...
    var options ={
        ...
        title:{
            text:'Anemia en Niños Menores de 3 Años en el '+" "+$("#estab option:selected").text(),
        },
        subtitle: {
